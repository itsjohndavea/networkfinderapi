...
// This will handle requests to the /api/v1/network/allcountrycode endpoint
export async function GET() {
    ...
    try {
        ...
        const countryCode = [...new Set(data.map(item => item.countryCode))];

        // Respond with the list of countries
        return NextResponse.json(
            { message: "Country Codes fetched successfully", status: 200, countryCode: countryCode },
            { status: 200 }
        );
    } catch (error: unknown) {
